        ...
        <div class="flex flex-col lg:flex-row gap-8 items-start">
            ...
            <div class="flex-1 min-w-0">
                ...
                <div class="bg-white border border-gray-100 rounded-2xl overflow-hidden shadow-sm">
                    ...
                    {{-- Body Konten --}}
                    <div class="px-8 py-6 text-gray-700 text-sm leading-relaxed space-y-5">

                        {{-- Intro --}}
                        <p>
                            <strong>{{ Str::before($article['content']['intro'], ' –') }} –</strong>
                            {{ Str::after($article['content']['intro'], ' – ') }}
                        </p>

                        {{-- Sections --}}
                        @foreach($article['content']['sections'] as $section)
                            <div>
                                <h2 class="font-bold text-gray-900 mb-2">{{ $section['heading'] }}</h2>
                                <p>{{ $section['body'] }}</p>
                            </div>
                        @endforeach
